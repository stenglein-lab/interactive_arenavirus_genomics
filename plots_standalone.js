...
plots.render = function(selector)
{
   ...
   function render_segments()
   {
      ...
      {
			// a subset - those highlighted
         segments = highlighted_segments.slice(0);
      }
      else
      {
		...
		console.log ("will render segments: " + segments);

      for (seg_index = 0; seg_index < segments.length; seg_index++) 
      {
         var segment_id = segments[seg_index];
   ...
   function draw_cartoon(segment_id)
   {
      ...
      var svg = drawing_area.append("svg")
       ...
       var file_name = "rod_seg_all.tsv"; // TODO - don't hardcode
